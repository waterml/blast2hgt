#!/usr/bin/perl -w
# $num=15, while $num=10 in lineage2idgroup3.pl
#read representives.txt and classify *.lin. 
#get n seqs based on subgroups hit's num(%).
#batch mode of lineage2idgroup3.pl, run once for all id
use Scalar::Util qw(looks_like_number);
die "usage: perl $0 [representatives.txt] [*.lineage] [id_file]\nOutput_to *.id.org\n" unless $ARGV[2];

open IDF, "$ARGV[2]" or die "FailToOpen $ARGV[2]\n"; #id from id_file
while(<IDF>){
	chomp; $ids{$_}=1;
}
close IDF;
#below should be same as lineage2idgroup3.pl
	...
	if($inputf==1){ #these lines are lineage, put them to hash
		...
		if(/\t.*?\t.*\t/){ ##1. standard *.lin
			...
			if(exists $tatxn{$arr[$#arr]}){ #1.1. with a lineage, classify to %tax
				...
				# if ($arr[3] >= 100){
				if ($arr[2] <= 1e-1){ #edit if need more strict citeria
					$acce=$arr[1];
...
foreach $seqid(sort keys %ha){
	...
	foreach my $subgroup (sort keys %{$ha{$seqid}}){
		...
		$num=15 if $num >15; #at most get 15 seqs per non-self subgroup
		$num=1 if $gp =~/^$tax_in_self{$selftax}/i; #self (within self tax) get 1 seq
		# $num=3 if ($gp eq $dgroup && $num <3); #donor subgroup at least 3
